...
public abstract class BaseFieldTag extends BaseInputTag {
	...
	/**
	 * Comma-delimited list of content types that a server processing this form will
	 * handle correctly. This property is defined only for the <code>file</code>
	 * tag, but is implemented here because it affects the rendered HTML of the
	 * corresponding &lt;input&gt; tag.
	 */
	...
	protected String renderInputElement() throws JspException {
		...
		prepareAttribute(results, "data-mini", getDataMini());

		// validate
		prepareAttribute(results, "required", getRequired());

		// datpickter
		prepareAttribute(results, "data-provide", getDataProvide());
		prepareAttribute(results, "data-date-language", getDataDateLanguage());
	 ...
	 *
	 * @param value The value to be formatted. <code>null</code> values will be
