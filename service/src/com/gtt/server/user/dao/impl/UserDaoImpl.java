...
public class UserDaoImpl extends CoreDaoImpl<User, Serializable> implements UserDao {
	...
	public List findLogin(String username, String password) throws DataAccessException {
		String sql = "SELECT user.id_user, user.username, user.password, user.user_firstname, "
				...
				+ "userposition.position_name, user.id_user_type FROM user "
				+ "inner join company on user.id_customer = company.id_company inner "
				+ "join userposition on user.id_user_position = userposition.id_user_position " + "WHERE username="
		...
		if (objectList != null && objectList.size() > 0) {
			for (Object[] obj : objectList) {
				...
				type_results.setId(Integer.parseInt(String.valueOf(obj[13])));
