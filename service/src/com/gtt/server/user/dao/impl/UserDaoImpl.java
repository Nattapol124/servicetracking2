...
public class UserDaoImpl extends CoreDaoImpl<User, Serializable> implements UserDao {
	...
	public List findLogin(String username, String password) throws DataAccessException {
		...
		String sql = "SELECT user.id_user, user.username,user.password,user.user_firstname,user.user_lastname,user.user_email,user.user_phone,user.id_customer,user.user_nickname,user.id_company,user.id_user_type FROM user WHERE username='"
	...
	public List findAddUserList(String company, String id_company) throws DataAccessException {
		System.out.println(company + " :test passvalue");
		String sql = "SELECT  user.id_user,user.username,user.user_firstname,user.user_lastname from user left JOIN userproject ON user.id_user=userproject.id_user AND userproject.id_project='"
				+ company + "' WHERE userproject.id_user IS null and user.id_customer='99' AND user.id_company='"
				+ id_company + "' ";
		List<User> results = new ArrayList<User>();
