...
public class OptionsXML extends TagSupport {
	...
	protected static String optionXlmPath = "com/demo/web/resource/option-data.xml";
	...
	public int doEndTag() throws JspException {
		...
		// Otherwise, use the separate iterators mode to render options
			...
			try {
				ClassPathResource cpr = new ClassPathResource(optionXlmPath);
				...
				Element e = doc.getElementById(this.getName());
				NodeList nl = e.getElementsByTagName("option");

				for (int i = 0; i < nl.getLength(); i++) {
					Node n = nl.item(i);
					NamedNodeMap columnAttributes = n.getAttributes();
					Node o = columnAttributes.getNamedItem("value");
					Node v = columnAttributes.getNamedItem("label");
					addOption(sb, o.getNodeValue(), v.getNodeValue(), selectTag.isMatched(o.getNodeValue()));
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
	 ...
	 * more than one <code>option</code> element element, but it cannot use the
	 * same <code>id</code> value. It's conceivable some sort of mechanism to
